...
def file_name_trees(leaves, seed, sites):
    return f"generated/experiment2/exp2_{leaves}_{seed}_{sites}/tree_comparison_results.csv"
...
def process_data_trees(leaves, seed, sites, flag):
    ...
    data.to_csv(
        "generated/experiment2/postprocessing/exp2_combined_data.csv",
...
def file_name_sizes(leaves, seed, sites):
    return f"generated/experiment2/exp2_{leaves}_{seed}_{sites}/tier_sizes.csv"

def process_data_sizes(leaves, seed, sites, flag):
    data = pd.read_csv(file_name_sizes(leaves, seed, sites))
    data['site_dep_param'] = 'ZIP' + data['site_dep_param'].astype(str)
    data['leaves'] = leaves
    data['independent_sites'] = sites
    data.to_csv(
        "generated/experiment2/postprocessing/exp2_combined_sizes.csv",
        mode="w" if flag else "a",
        header=flag,
        index=False
    )
